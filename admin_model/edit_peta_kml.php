...
if (!isset($_FILES['kml']) || $_FILES['kml']['error'] !== 0) {
    $pesans[] = "File kml baru wajib diunggah.";
}
...
if (empty($pesans)) {
    ...
    $data = read($conn, "SELECT kml FROM data_peta WHERE id = ?", "i", [$id]);
    if (!$data) {
        $pesans[] = "Data tidak ditemukan.";
    } else {
        $oldFilename = $data[0]['kml'];
        $oldPath = '../storage/kml/' . $oldFilename;

        // Siapkan nama file baru
        $originalName = pathinfo($_FILES['kml']['name'], PATHINFO_FILENAME);
        $extension = strtolower(pathinfo($_FILES['kml']['name'], PATHINFO_EXTENSION));
        $datePrefix = date('Ymd_His');
        $safeName = preg_replace('/[^a-zA-Z0-9_-]/', '_', $originalName);
        $newFilename = "{$datePrefix}_{$safeName}.{$extension}";

        // Upload file baru
        $uploadResult = upload_file($_FILES['kml'], ['kml'], '../storage/kml/', $newFilename);
        ...
        if (!$uploadResult['success']) {
            ...
            $query = "UPDATE data_peta SET kml = ? WHERE id = ?";
