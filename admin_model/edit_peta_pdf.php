...
if (!isset($_FILES['pdf']) || $_FILES['pdf']['error'] !== 0) {
    $pesans[] = "File pdf baru wajib diunggah.";
}
...
if (empty($pesans)) {
    ...
    $data = read($conn, "SELECT pdf FROM data_peta WHERE id = ?", "i", [$id]);
    if (!$data) {
        $pesans[] = "Data tidak ditemukan.";
    } else {
        $oldFilename = $data[0]['pdf'];
        $oldPath = '../storage/pdf/' . $oldFilename;
        ...
        // Upload file baru
        $uploadResult = upload_file($_FILES['pdf'], ['pdf'], '../storage/pdf/', $newFilename);
        ...
        if (!$uploadResult['success']) {
            ...
            $query = "UPDATE data_peta SET pdf = ? WHERE id = ?";
